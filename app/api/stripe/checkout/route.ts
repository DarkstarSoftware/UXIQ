...
export async function POST() {
  ...
  if (!priceId) return NextResponse.json({ error: 'Missing STRIPE_PRO_PRICE_ID.' }, { status: 500 });
  ...
  if (userError || !user) {
    return NextResponse.json(
      { error: 'You must be signed in before upgrading.', url: `${siteUrl}/auth/login?redirect=/pricing` },
      { status: 401 },
    );
  }
